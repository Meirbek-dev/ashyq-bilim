...
class TestRBACResolution:
    @pytest.mark.parametrize(
        ...
        [
            ...
            ({"course:read:platform"}, "course:read", {"resource_owner_id": 999}, True),  # platform beats everything

            # --- Own Scope Tests ---
            ({"course:read:own"}, "course:read", {"resource_owner_id": 1}, True),       # I am the owner
            ({"course:read:own"}, "course:read", {"resource_owner_id": 2}, False),      # Not the owner
            ({"course:read:own"}, "course:read", {"is_owner": True}, True),             # Explicit ownership flag
            ({"course:read:own"}, "course:read", {"is_owner": False}, False),           # Explicit not-owner flag

            # --- Assigned Scope Tests ---
            ({"course:read:assigned"}, "course:read", {"is_assigned": True}, True),     # I am assigned
            ({"course:read:assigned"}, "course:read", {"is_assigned": False}, False),   # Not assigned
            ({"course:read:assigned"}, "course:read", {"resource_owner_id": 1}, False), # Ownership does not imply assignment implicitly here

            # --- Combined Scopes (Highest wins) ---
            ({"course:read:own", "course:read:assigned"}, "course:read", {"is_assigned": True, "resource_owner_id": 2}, True),
            ({"course:read:own", "course:read:assigned"}, "course:read", {"is_assigned": False, "resource_owner_id": 1}, True),
            ({"course:read:own", "course:read:assigned"}, "course:read", {"is_assigned": False, "resource_owner_id": 2}, False),
            ...
            ({"course:read:own"}, "course:read", {"resource_owner_id": 0}, False),  # User 0 cannot own things
        ]
    )
    def test_permission_resolution(
        ...
        user_id = 0 if kwargs.get("resource_owner_id") == 0 and list(granted_perms)[0].endswith(":own") else 1
        if user_id == 0:
...
class TestRBACSecurityEdgeCases:
    def test_role_escalation_via_wildcards_is_supported(self, checker: PermissionChecker) -> None:
    ...
    def test_escalation_attempt_with_malformed_owner(self, checker: PermissionChecker) -> None:
